...
export const CurrencyProvider = ({ children, baseTokenSymbol = null }) => {
  ...
  useEffect(() => {
    if (!baseTokenSymbol && !selectedCurrency) {
      // Use config-driven approach via useUpdateCurrencyFromConfig where possible, 
      // avoiding strict hardcoding here to allow flexibility.
      // But for initial load without config, we might need a safe default.
      const isMainnet = chain?.id === 1;
      // Only set if we really have no other info
      if (chain?.id) {
