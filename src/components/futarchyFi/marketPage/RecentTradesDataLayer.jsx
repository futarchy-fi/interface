...
const RecentTradesDataLayer = ({
          ...
          if (config.BASE_TOKENS_CONFIG.company) {
            ...
            if (companyTokens.address) {
              tradeHistoryFetcher.tokenCache.set(companyTokens.address.toLowerCase(), {
                data: {
                  symbol: companyTokens.symbol || 'COMPANY',
                  name: companyTokens.name || companyTokens.symbol || 'COMPANY',
                  decimals: 18
                },
                timestamp: Date.now()
              });
            ...
            if (yesTokenAddr) {
              tradeHistoryFetcher.tokenCache.set(yesTokenAddr.toLowerCase(), {
                data: {
                  symbol: `YES_${companyTokens.symbol || 'COMPANY'}`,
                  name: `YES ${companyTokens.name || companyTokens.symbol || 'COMPANY'}`,
                  decimals: 18
                },
                timestamp: Date.now()
              });
            ...
            if (noTokenAddr) {
              tradeHistoryFetcher.tokenCache.set(noTokenAddr.toLowerCase(), {
                data: {
                  symbol: `NO_${companyTokens.symbol || 'COMPANY'}`,
                  name: `NO ${companyTokens.name || companyTokens.symbol || 'COMPANY'}`,
                  decimals: 18
                },
                timestamp: Date.now()
              });
  ...
  const companySymbols = useMemo(() => {
    ...
    addSymbol(config?.BASE_TOKENS_CONFIG?.company?.symbol);
