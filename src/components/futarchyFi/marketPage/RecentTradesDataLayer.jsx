...
const RecentTradesDataLayer = ({
  ...
  useEffect(() => {
    const fetchTrades = async () => {
      ...
      try {
        // Build fetch params - ALWAYS fetch ALL trades (no userAddress filter)
        const fetchParams = {
    ...
    fetchTrades();
  }, [config?.proposalId, config?.MARKET_ADDRESS, limit]); // Only refetch when proposal or limit changes
  ...
  useEffect(() => {
    ...
    const channel = supabaseClientRef.current
      ...
      .on('postgres_changes', {
        ...
        console.log('[RecentTradesDataLayer] 🔴 RAW PAYLOAD RECEIVED:', JSON.stringify(payload, null, 2));
        ...
        try {
          ...
          setAllTrades(prevTrades => {
            ...
            const isDuplicate = prevTrades.some(trade => {
              const existingId = trade.id || trade.transactionLink ||
                                `${trade.blockNumber}-${trade.logIndex}`;
              return existingId === uniqueTradeId ||
                     (trade.transactionLink && trade.transactionLink.includes(payload.new.evt_tx_hash));
            });

            if (isDuplicate) {
              console.log('[RecentTradesDataLayer] ⚠️ Duplicate trade detected, skipping:', uniqueTradeId);
              return prevTrades;
            }
            ...
            <table className="w-full">
              <tbody>
                {filteredTrades.map((trade, index) => {
                  ...
                  return (
                    <tr
                      key={trade.transactionLink || trade.evt_tx_hash || `${trade.blockNumber}-${index}`}
                      className={`border-b border-futarchyGray62 dark:border-futarchyDarkGray42 hover:bg-futarchyGray3 dark:hover:bg-futarchyGray3/20 transition-all duration-500 h-[60px] ${
                        isNewTrade ? 'animate-new-trade-highlight' : ''
                      }`}
                    >
                      <td className="px-4 w-[200px]">
                        <div className="flex flex-col gap-1">
                          <div className="inline-flex overflow-hidden w-[160px]">
                            <div className={`w-[80px] px-3 py-1 text-sm font-medium text-center rounded-l-full ${
                              isYes
                            ...
                            <div className={`w-[80px] px-3 py-1 text-sm font-medium text-center rounded-r-full ${
                              !isBuy
          ...
          <div className="flex flex-col gap-3">
            {filteredTrades.map((trade, index) => {
              ...
              return (
                <div
                  key={trade.transactionLink || trade.evt_tx_hash || `${trade.blockNumber}-${index}`}
                  className={`bg-futarchyGray3 dark:bg-futarchyDarkGray3 border border-futarchyGray62 dark:border-futarchyDarkGray42 rounded-lg p-3 ${
                    isNewTrade ? 'animate-new-trade-highlight' : ''
                  }`}
                >
                  {/* Trader Address */}
                  ...
                  <div className="flex flex-col gap-2.5">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <div className="inline-flex overflow-hidden">
                        <div className={`px-3 py-1 text-xs font-medium text-center rounded-l-full ${
                          isYes
                            ? 'bg-futarchyBlue4 text-futarchyBlue11 border border-futarchyBlue6 dark:bg-transparent dark:text-futarchyBlue9 dark:border-futarchyBlue9'
                            : isNo
                            ? 'bg-futarchyGold4 text-futarchyGold11 border border-futarchyGold6 dark:bg-transparent dark:text-futarchyGold7 dark:border-futarchyGold7'
                        ...
                        <div className={`px-3 py-1 text-xs font-medium text-center rounded-r-full ${
                          !isBuy
                            ? 'bg-futarchyTeal3 text-futarchyTeal9 border border-futarchyTeal5 dark:bg-transparent dark:text-futarchyTeal7 dark:border-futarchyTeal7'
                            : 'bg-futarchyCrimson4 text-futarchyCrimson11 border border-futarchyCrimson6 dark:bg-transparent dark:text-futarchyCrimson9 dark:border-futarchyCrimson9'
                        }`}>
