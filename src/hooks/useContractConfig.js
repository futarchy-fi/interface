    ...
    const fetchContractConfig = async () => {
      try {
        ...
        const transformedConfig = {
          ...
          // Priority: 1) metadata.base_pool (if has address), 2) proposalSpecificBasePool, 3) chain-specific hardcoded, 4) defaults
          BASE_POOL_CONFIG: metadata.base_pool?.address ? {
            // Highest priority: Use base_pool from metadata if provided with address
            ...
            // Second priority: Proposal-specific base pool override
            ...
            // Third priority for chain 1: Hardcoded Ethereum mainnet default
            ...
            // Fourth priority: Force test base pool (for non-mainnet chains)
            ...
            // Final fallback: Gnosis Chain default base pool
          ...
          marketInfo: {
            ...
            // TWAP configuration
            twapStartTimestamp: metadata?.twapStartTimestamp || null,
            twapDurationHours: metadata?.twapDurationHours || 24,
            twapDescription: metadata?.twapDescription || null
          },
