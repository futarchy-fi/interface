...
function logDebug(message, data = {}) {
  ...
  console.log(`\n🔍 [SWAPR-DEBUG #${callCounter}] ${message}`, {
    timestamp,
    ...data,
...
function getCallStats() {
  return {
    ...
    callHistory,
    uniquePoolAddresses: [...new Set(callHistory.map(c => c.poolAddress).filter(Boolean))],
    uniqueTokenPairs: [...new Set(callHistory.map(c => c.tokenPair).filter(Boolean))]
  };
}

// Export for debugging in browser console
export function getSwaprDebugStats() {
  const stats = getCallStats();
  console.table({
    'Total Calls': stats.totalCalls,
    'Unique Pools': stats.uniquePoolAddresses.length,
    'Unique Token Pairs': stats.uniqueTokenPairs.length,
    'Call History Length': stats.callHistory.length
  });
  console.log('📊 Unique Token Pairs:', stats.uniqueTokenPairs);
  console.log('📊 Unique Pool Addresses:', stats.uniquePoolAddresses);
  console.log('📊 Recent 10 Calls:', stats.callHistory.slice(-10));
  return stats;
}

// Make it accessible from browser console
if (typeof window !== 'undefined') {
  window.getSwaprDebugStats = getSwaprDebugStats;
  console.log('🔍 Debug function available: window.getSwaprDebugStats()');
}
...
export async function getSwaprV3QuoteWithPriceImpact({
  ...
  try {
    ...
    const callerLine = stackLines[2] || 'Unknown caller';

    logDebug('🚀 FUNCTION ENTRY - getSwaprV3QuoteWithPriceImpact called', {
      callNumber: callCounter,
    ...
    logDebug('⚠️ ABOUT TO CALL SwaprV3Trade.getQuote - THIS MAY TRIGGER MULTIPLE RPC CALLS', {
      inputAmount: inputAmount.toExact(),
      outputCurrency: outputToken.symbol,
      slippageBps,
      providerType: provider?.constructor?.name,
      useAggregationOptimization: true,
      tokenPair
    });
    ...
    logDebug('✅ SwaprV3Trade.getQuote COMPLETED', {
      ...
      routeHops: trade.route?.path?.length,
      tokenPair
    });
