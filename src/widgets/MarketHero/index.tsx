...
export const MarketHeroWidget = () => {
    const { getMarket } = useMarketService(MOCK_CONFIG.MARKET.DEFAULT_ID);
    ...
    React.useEffect(() => {
        const fetch = async () => {
            try {
                // Hardcoded ID for now - in real app would come from params/props
                console.log("Fetching market:", MOCK_CONFIG.MARKET.DEFAULT_ID);
                const data = await getMarket(MOCK_CONFIG.MARKET.DEFAULT_ID);
                if (!data) {
            ...
            <div className="container mx-auto px-5 flex-grow flex flex-col justify-center">
                <div className="grid grid-cols-1 lg:grid-cols-3 transition-all duration-300 ease-in-out py-8 lg:py-12">
                    ...
                    <div className="lg:col-span-2 space-y-3 py-4 lg:space-y-4 border-b-2 border-[#2A2A2A] lg:border-b-0 lg:border-r lg:pr-6 transition-all duration-300 ease-in-out lg:py-6">
                        ...
                        {/* Title (Dynamic) */}
                        <h1 className="font-semibold text-white leading-tight min-h-[1.5rem] transition-all duration-300 ease-in-out text-xl lg:text-3xl">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-y-4 text-left transition-all duration-300 ease-in-out">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <div className="flex flex-col items-start">
                                    <span className="text-sm lg:text-base font-semibold text-[#00A89D]">+47.70%</span>
                                    <span className="text-xs text-white/70">{MARKET_HERO_MESSAGES.LABELS.IMPACT}</span>
                                </div>
                            </div>
