...
async function generateSEOData() {
  ...
  // Load mapped SEO configuration
  const mappedSeoPath = path.join(process.cwd(), 'src', 'config', 'mapped-seo.json');
  let mappedSeo = {};
  if (fs.existsSync(mappedSeoPath)) {
    try {
      mappedSeo = JSON.parse(fs.readFileSync(mappedSeoPath, 'utf8'));
      logToFile('INFO', `Loaded mapped SEO configuration with ${Object.keys(mappedSeo).length} entries`);
    } catch (e) {
      logToFile('ERROR', 'Failed to load mapped SEO configuration', { error: e.message });
    }
  }
  ...
  try {
    ...
    for (let i = 0; i < markets.length; i++) {
      ...
      let title = await generateTitle(market, aiSEO);
      let description = await generateDescription(market, aiSEO);
      let image = generateImage(market);

      // Check if market is mapped in mapped-seo.json and prioritize the image
      if (mappedSeo[marketId]) {
        // If the value is a string, it's just the image path
        if (typeof mappedSeo[marketId] === 'string') {
          image = mappedSeo[marketId];
          logToFile('INFO', `Using mapped image for market ${marketId}: ${image}`);
        } else if (mappedSeo[marketId].image) {
          // Backward compatibility if we ever go back to object format
          image = mappedSeo[marketId].image;
          logToFile('INFO', `Using mapped image for market ${marketId}: ${image}`);
          if (mappedSeo[marketId].title) title = mappedSeo[marketId].title;
          if (mappedSeo[marketId].description) description = mappedSeo[marketId].description;
        }
      }
      ...
      marketsConfig[marketId] = {
        ...
        twitter: {
          ...
          title: `${title} | Futarchy.fi`,
          description,
          image: image.startsWith('http') ? image : `https://app.futarchy.fi${image}`
        },
        keywords,
        category,
        isActive,
