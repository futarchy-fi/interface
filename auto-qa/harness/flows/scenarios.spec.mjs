...
const loadedScenarios = await Promise.all(
    scenarioFiles.map(async (file) => {
        ...
        assertKnownScenarioTiers(file, mod.default);
        return { file, scenario: mod.default };
    }),
);

const requestedTier = process.env.HARNESS_SCENARIO_TIER?.trim() || '';
const requestedIds = parseScenarioIds(process.env.HARNESS_SCENARIO_IDS);
const scenarios = loadedScenarios.filter(({ file, scenario }) =>
    scenarioMatchesTier(scenario, requestedTier) &&
    scenarioMatchesIds(file, requestedIds),
);

if (requestedTier && scenarios.length === 0) {
    throw new Error(`HARNESS_SCENARIO_TIER=${requestedTier} matched zero scenarios`);
}
if (requestedIds.size > 0 && scenarios.length === 0) {
    throw new Error(`HARNESS_SCENARIO_IDS=${[...requestedIds].join(',')} matched zero scenarios`);
}
