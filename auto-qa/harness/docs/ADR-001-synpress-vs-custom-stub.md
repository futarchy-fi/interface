...
- **Negative**
  - We do not exercise the real MetaMask connection UX. Regressions in
    ...
    browser suite or a thin Synpress-based browser suite gated to nightly.
...
- **Synpress nightly browser suite (defer)** — keep on the table as a *separate* thin suite if/when we want to assert real MetaMask connection UX. Out of scope for Phases 0–6.
