...
// in-process stub the fork-state tests use).
//
// **Not in the default unit run** — requires `anvil` listening on
// `HARNESS_ANVIL_URL` (defaults to localhost:8546). When that isn't
// reachable, the test SKIPS rather than fails, so CI runs
...
//
// What this catches that the stub-based unit check can't:
