...
- [x] **4d-by-layer-script — catalog ergonomics**. Added
      `npm run scenarios:by-layer` on the api side (35-line
      ...
      per-layer detail blocks. At 55 invariants the dry-
      run flat catalog is hard to scan; this answers
      "what does X layer cover?" and "where's the catalog
      growing?" in one command. Surfaces authoritative
      layer breakdown: api=10, api↔candles=4,
      api↔registry=2, orchestrator↔candles=21,
      orchestrator↔chain=10, orchestrator↔registry=8
      (= 55 total). Api commit: `e9d2954`. (interface
      side mirrors docs only — the script lives where
      the orchestrator lives.)
- [ ] **4d-scenarios-more — add remaining invariants**.
      **Now 57 invariants (api side)**: 10 api + 5
      api↔candles + 3 api↔registry + 21 orchestrator↔candles
      + 8 orchestrator↔registry + 10 orchestrator↔chain
      (per `scenarios:by-layer`). 197 smoke tests green.
      ...
      catches that drift. For ANY introspection failure,
      the four-probe truth table pinpoints layer
      (api/direct) × indexer (registry/candles).
      ...
      with the DIRECT sister to pinpoint WHICH layer broke:
      api✗+direct✓ = api stripped introspection;
      api✓+direct✗ = indexer schema regressed (api correctly
      forwarded the broken schema); both✗ = indexer is
      root cause. The api↔registry layer (previously
      thinnest at 2) is now at 3.

      `candleOHLCAllRowsConsistent` (previous slice) —
      ...
      + TIME-WARP. Without time-warp, ANY scenario
      involving a time-gated state transition (resolution
      after deadline, TWAP window, vote-weight decay)
      cannot run at all. Same support profile as
      evm_snapshot (ganache lineage; geth/erigon/reth
      lack it). Side effect (next-block timestamp set to
      now+86400) is benign — no block mined in the probe.
      ...
      account) + snapshot/revert (roll back state). evm_snapshot
      is part of the GANACHE LINEAGE (anvil + hardhat
      support; geth/erigon/reth don't). Distinct failure
      modes from impersonation: anvil_* missing → wrong
      dev client (hardhat); evm_* missing → real client
      (geth/erigon/reth). Also catches subsystem-broken
      case: method registered but returns null/non-hex
      (calling evm_revert with that silently fails).
