...
| PRs classified | 42 |
| Tests added | 437 (4 extractor-sanity + 2 graphql-compat + 5 endpoint-liveness + 10 url-shapes + 2 dead-references + 6 liquidity-math + 7 slippage-math + 8 snapshot-id-extraction + 3 pagination-first-cap + 6 twap-window + 14 impact-formula + 11 proposal-resolution-bucketing + 6 footer-links + 16 format-number + 5 asset-refs + 19 precision-formatter + 20 image-utils + 10 retry-backoff + 11 contract-addresses + 47 json-config-validity + 11 subgraph-endpoints + 16 rpc-config + 11 sqrt-price-x96 + 12 sdai-rate-config + 21 snapshot-api + 18 safe-tx-receipt + 18 is-safe-wallet + 24 best-rpc-cache + 14 seeded-random + 19 proposal-doc-store + 26 futarchy-quote-helper + 35 algebra-quoter — all passing) |
...
| Code-smell surfaced (cross-file dup) | `src/components/futarchyFi/marketPage/MarketCharts.jsx:28-37` duplicates the `SeededRandom` class inline (the canonical lives at `src/utils/seededRandom.js`) — pinned cross-file equality in `seeded-random.test.mjs`; drift means two charts of the same proposal produce different visual jitter. |
