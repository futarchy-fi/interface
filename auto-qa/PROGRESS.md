...
| PRs classified | 42 |
| Tests added | 675 (4 extractor-sanity + 2 graphql-compat + 5 endpoint-liveness + 10 url-shapes + 2 dead-references + 6 liquidity-math + 7 slippage-math + 8 snapshot-id-extraction + 3 pagination-first-cap + 6 twap-window + 14 impact-formula + 11 proposal-resolution-bucketing + 6 footer-links + 16 format-number + 5 asset-refs + 19 precision-formatter + 20 image-utils + 10 retry-backoff + 11 contract-addresses + 47 json-config-validity + 11 subgraph-endpoints + 16 rpc-config + 11 sqrt-price-x96 + 12 sdai-rate-config + 21 snapshot-api + 18 safe-tx-receipt + 18 is-safe-wallet + 24 best-rpc-cache + 14 seeded-random + 19 proposal-doc-store + 26 futarchy-quote-helper + 35 algebra-quoter + 31 algebra-pool-price + 31 subgraph-pool-fetcher + 31 subgraph-bulk-price-fetcher + 26 sushiswap-helper + 31 unified-balance-fetcher + 33 swapr-sdk + 54 subgraph-trades-client — all passing) |
| Misleading comments surfaced | `src/utils/snapshotApi.js` quorum-percent docstring claims `89.7014 → 89.6%` but actual code yields `89.7%` (floor at 1-decimal precision, not 0). Pinned in snapshot-api.test.mjs |
| Dead-code surfaced | `src/utils/formatters.js` (formatForDisplay/formatPrice/formatBalance/formatPercentage) has zero callers; also uses ethers v5 `ethers.utils.parseUnits` API while the installed ethers is v6 — would silently throw if ever called (try/catch returns '0'). **iter 35**: `src/utils/MarkdownParser.js` has zero importers AND executes `console.log(html)` at module evaluation (lines 60-62); pinned in `seeded-random.test.mjs` as a hazard ratchet. **NEW (iter 36)**: `src/utils/proposalDocumentationStore.js` has zero importers AND its setter contains 7 `console.log` calls (would flood logs if ever wired in); pinned in `proposal-doc-store.test.mjs` along with the silent-XSS surface (script/style/iframe pass through verbatim) and the "<h1 class=...>" attribute-handling gap. |
| Code-smell surfaced (cross-file dup) | `src/components/futarchyFi/marketPage/MarketCharts.jsx:28-37` duplicates the `SeededRandom` class inline (the canonical lives at `src/utils/seededRandom.js`) — pinned cross-file equality in `seeded-random.test.mjs`; drift means two charts of the same proposal produce different visual jitter. **iter 39**: `algebraQuoter.js` and `getAlgebraPoolPrice.js` BOTH define `POOL_ABI` for `globalState` BUT WITH DIFFERENT TUPLE SHAPES — `algebraQuoter` has 7 fields (older Algebra V3: `fee`, `timepointIndex`, `communityFeeToken0`, `communityFeeToken1`); `getAlgebraPoolPrice` has 6 fields (newer Algebra Integral: `lastFee`, `pluginConfig`, single `communityFee`). The two files target DIFFERENT Algebra contract versions — pinned in `algebra-pool-price.test.mjs` so any unification would force a deliberate decision. **NEW (iter 44)**: `sqrtPriceX96ToPrice` is now duplicated **FOUR TIMES** across `algebraQuoter.js`, `FutarchyQuoteHelper.js` (as `calculatePriceFromSqrt`), `getAlgebraPoolPrice.js` (inline `(Number(sqrt) ** 2) / 2 ** 192`), and `swaprSdk.js` (as `sqrtPriceX96ToPrice`). All pinned to the same canonical Algebra V3 formula but distinct copies. Worth a future consolidation. |
...
| **Real bugs surfaced** | **16 broken GraphQL queries** (see `auto-qa/fixtures/known-graphql-failures.json`). **iter 42**: `src/futarchyJS/futarchy.js:1238` calls `fetchSushiSwapRoute(fromToken, toToken, parsedAmount)` with POSITIONAL args, but the helper expects a destructured `{tokenIn, tokenOut, amount, ...}` object — destructuring a string yields undefined for all keys → "Invalid amount provided" throw. The other call site in `ShowcaseSwapComponent.jsx:1125` uses the correct destructured form. May be dead code OR a latent bug. Pinned in `sushiswap-helper.test.mjs::hazard H1`. Plus a hardcoded feeReceiver address `0xca226bd9c754F1283123d32B2a7cF62a722f8ADa` leaked in a `console.log` line in `sushiswapHelper.js:91`. **NEW (iter 44)**: `src/utils/swaprSdk.js` ALGEBRA_SUBGRAPH URL bakes in a hardcoded Graph API key `8b2690ffdd390bad59638b894ee8d9f6` in the path (similar pattern to the CoinGecko key in spot-price.js). Plus `window.getSwaprDebugStats` global pollution + module-load `console.log` of "Debug function available". Pinned in `swapr-sdk.test.mjs::hazard H1/H2`. |
