...
# Script to test the Liquidity Calculation Engine implementation
...
# Run the test script
echo -e "${BLUE}Compiling and running the test...${NC}"

# Use RPC_URL if GNOSIS_RPC_URL is not set
if [ -z "$GNOSIS_RPC_URL" ]; then
  if [ -z "$RPC_URL" ]; then
    echo -e "${RED}Error: Neither GNOSIS_RPC_URL nor RPC_URL environment variable is set${NC}"
    echo "Please set one of them with: export RPC_URL='your-rpc-url'"
    exit 1
  else
    echo -e "${BLUE}Using RPC_URL environment variable${NC}"
    GNOSIS_RPC_URL="$RPC_URL"
  fi
fi

# Make sure CHAIN_ID is set
if [ -z "$CHAIN_ID" ]; then
  echo -e "${BLUE}Setting CHAIN_ID=100 for Gnosis Chain${NC}"
  export CHAIN_ID=100
fi
...
if [[ $output == *"Liquidity Calculation Test Completed Successfully!"* ]]; then
  echo -e "${GREEN}✓ Liquidity Calculation Engine test passed!${NC}"
else
  echo -e "${RED}✗ Liquidity Calculation Engine test failed!${NC}"
  echo "$output"
  exit 1
fi
