...
async function removeAllFromPool(poolAddress, percentageToRemove = 100, autoMode = false) {
  ...
  try {
    ...
    // Add timeout wrapper for the balance call
    const timeoutPromise = new Promise((_, reject) => 
      setTimeout(() => reject(new Error('Timeout: balanceOf call took too long')), 30000)
    );
    
    let balance;
    try {
      balance = await Promise.race([
        pm.balanceOf(wallet.address),
        timeoutPromise
      ]);
      console.log(`Found ${balance} total positions in wallet`);
    } catch (error) {
      console.error("❌ Error fetching positions:", error.message);
      console.log("Attempting to reconnect to RPC...");
      
      // Try to create a new provider connection
      const newProvider = new ethers.JsonRpcProvider(RPC_URL);
      const newWallet = new ethers.Wallet(PRIVATE_KEY, newProvider);
      const newPm = new ethers.Contract(POSITION_MGR, pmAbi, newWallet);
      
      try {
        balance = await newPm.balanceOf(wallet.address);
        console.log(`Reconnected successfully. Found ${balance} positions`);
        // Update global pm instance
        pm = newPm;
      } catch (retryError) {
        console.error("❌ Failed to fetch positions after retry:", retryError.message);
        return null;
      }
    }
    ...
    // Cache for pool lookups to avoid repeated calls
    const poolCache = new Map();
    
    // Process positions in batches to avoid overwhelming the RPC
    const batchSize = 10;
    const totalPositions = Number(balance);
    console.log(`Scanning ${totalPositions} positions (this may take a while)...`);
    
    // Get target pool's token pair for faster comparison
    let targetTokens = null;
    try {
      const poolContract = new ethers.Contract(poolAddress, algebraPoolAbi, provider);
      const token0 = await poolContract.token0();
      const token1 = await poolContract.token1();
      targetTokens = { token0: token0.toLowerCase(), token1: token1.toLowerCase() };
      console.log(`Target pool tokens: ${token0} / ${token1}`);
    } catch (err) {
      console.warn(`Could not fetch pool tokens, will use slower lookup method`);
    }
    
    for (let i = 0; i < totalPositions; i++) {
      // Show progress every 10 positions
      if (i > 0 && i % 10 === 0) {
        console.log(`  Progress: ${i}/${totalPositions} positions scanned...`);
      }
      ...
      try {
        ...
        // Quick check using target tokens if available
        if (targetTokens) {
          const posTokens = { token0: token0.toLowerCase(), token1: token1.toLowerCase() };
          if (posTokens.token0 !== targetTokens.token0 || posTokens.token1 !== targetTokens.token1) {
            continue; // Skip positions that don't match target pool tokens
          }
        }
        
        // Check if this position belongs to the specified pool (with caching)
        const cacheKey = `${token0}-${token1}`;
        let currentPoolAddr;
        if (poolCache.has(cacheKey)) {
          currentPoolAddr = poolCache.get(cacheKey);
        } else {
          currentPoolAddr = await poolByPair(token0, token1);
          poolCache.set(cacheKey, currentPoolAddr);
        }
        ...
        console.warn(`  ⚠️ Error scanning position ${i}: ${posError.message}`);
