...
async function formatEthereumTrades() {
    ...
    try {
        ...
        // For Ethereum mainnet tokens, we'll need to determine the token types
        // Based on the pattern, these look like futarchy tokens
        const TOKEN_MAPPING = {
            // First trade tokens
            '0x03cbdbecda0c93ee324f4900d1514bc2672fc51a': { symbol: 'YES_TOKEN', name: 'YES Token', decimals: 18 },
            '0x5e31218dc0696de0f2432bd0021768e7acc13bf7': { symbol: 'BASE_TOKEN', name: 'Base Token', decimals: 18 },
            // Second trade tokens
            '0x192e4580d85dc767f81f8ad02428f042e3c1074e': { symbol: 'NO_TOKEN', name: 'NO Token', decimals: 18 },
            '0x87f94faba3e8fd5fbb9f49f7e9ab24e8fc6e7b7e': { symbol: 'BASE_TOKEN', name: 'Base Token', decimals: 18 },
        };

        // Format each trade
        const formattedTrades = rawData.map((trade) => {
            // Get token info
            const token0Info = TOKEN_MAPPING[trade.token0.toLowerCase()] || {
                symbol: `TOKEN_${trade.token0.slice(2, 8).toUpperCase()}`,
                decimals: 18
            };
            const token1Info = TOKEN_MAPPING[trade.token1.toLowerCase()] || {
...
// Run the formatter
formatEthereumTrades().catch(error => {
    console.error(JSON.stringify({
        error: error.message
    }, null, 2));
});