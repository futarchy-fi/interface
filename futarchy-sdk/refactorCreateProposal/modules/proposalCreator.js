...
class ProposalCreator {
  ...
  async createProposal(params) {
    ...
    // Build transaction options with gas price for networks that require it
    const txOptions = {
      gasLimit: finalGasLimit,
      ...gasOptions
    };

    // Apply configured gas price ONLY for Gnosis Chain (100) which requires minimum priority fee
    // Other chains (Ethereum, Polygon) work fine with auto gas pricing
    const isGnosisChain = constants.NETWORK.CHAIN_ID === 100;
    if (isGnosisChain && constants.GAS_PRICE_GWEI && !gasOptions.gasPrice && !gasOptions.maxFeePerGas) {
      ...
      console.log(`  ⛽ Gas Price: ${constants.GAS_PRICE_GWEI} Gwei (Gnosis Chain requirement)`);
    }
