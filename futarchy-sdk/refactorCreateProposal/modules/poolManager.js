...
class PoolManager {
  ...
  async createPool(token0Address, token1Address, amount0, amount1, gasOptions = {}, logicalPrice = null, feeTier = 3000) {
    ...
    // Use baseFee + priority for maxFeePerGas (more conservative)
    const baseFee = feeData.gasPrice ?? ethers.parseUnits('1', 'gwei');
    let maxFeePerGas = baseFee + maxPriorityFeePerGas;

    // Boost gas price by 10% for faster confirmation
    maxFeePerGas = (maxFeePerGas * 110n) / 100n;
    maxPriorityFeePerGas = (maxPriorityFeePerGas * 110n) / 100n;

    // Check ETH balance and calculate affordable gas settings
    const ethBalance = await this.wallet.provider.getBalance(this.wallet.address);
    const currentNonce = await this.wallet.provider.getTransactionCount(this.wallet.address, 'pending');
    console.log(`  💰 Wallet ETH balance: ${ethers.formatEther(ethBalance)} ETH`);
    console.log(`  🔢 Current nonce: ${currentNonce}`);

    // Use configured gas limit from chain config (e.g., 16M for Gnosis)
    const createGasLimit = gasOptions.gasLimit || BigInt(constants.GAS_SETTINGS.CREATE_POOL);
    ...
    console.log(`  ⛽ Gas limit: ${createGasLimit} (needs ${gasNeeded} + 10% margin)`);
    console.log(`  ⛽ Network gas prices (with 10% boost): Base ${ethers.formatUnits(baseFee, 'gwei')} Gwei, Priority ${ethers.formatUnits(maxPriorityFeePerGas, 'gwei')} Gwei, Max ${ethers.formatUnits(maxFeePerGas, 'gwei')} Gwei`);
    console.log(`  ⛽ Max affordable gas price: ${ethers.formatUnits(maxAffordableGasPrice, 'gwei')} Gwei`);

    // Apply hard cap from config if set
    const maxFeePerGasGwei = constants.CHAIN_CONFIG?.maxFeePerGasGwei;
    if (maxFeePerGasGwei) {
      const hardCap = ethers.parseUnits(maxFeePerGasGwei.toString(), 'gwei');
      if (maxFeePerGas > hardCap) {
        console.log(`  🛑 Hard cap: Reducing maxFeePerGas from ${ethers.formatUnits(maxFeePerGas, 'gwei')} to ${maxFeePerGasGwei} Gwei (config limit)`);
    ...
    if (maxFeePerGas > maxAffordableGasPrice) {
      console.log(`  ⚠️  Budget cap: Reducing maxFeePerGas from ${ethers.formatUnits(maxFeePerGas, 'gwei')} to ${ethers.formatUnits(maxAffordableGasPrice, 'gwei')} Gwei to fit budget`);
      maxFeePerGas = maxAffordableGasPrice;
      // Also reduce priority fee proportionally to maintain the boost
      maxPriorityFeePerGas = maxPriorityFeePerGas < maxAffordableGasPrice ? maxPriorityFeePerGas : maxAffordableGasPrice / 2n;
    }

    const estimatedCost = (createGasLimit * maxFeePerGas);
    ...
    try {
      ...
      console.log(`  Error details: ${err.shortMessage || err.message}`);
      ...
      const factory = new ethers.Contract(ethers.getAddress(factoryAddress), factoryAbi, this.wallet);
      let newPoolAddress;
      try {
    ...
    console.log(`  🔢 Nonce used: ${tx.nonce}`);
  ...
  async mintPosition(params) {
    ...
    const baseFee = feeData.gasPrice ?? ethers.parseUnits('1', 'gwei');
    let maxFeePerGas = baseFee + maxPriorityFeePerGas;
    console.log(`  ⛽ Gas prices: Base ${ethers.formatUnits(baseFee, 'gwei')} Gwei, Priority ${ethers.formatUnits(maxPriorityFeePerGas, 'gwei')} Gwei, Max ${ethers.formatUnits(maxFeePerGas, 'gwei')} Gwei`);
