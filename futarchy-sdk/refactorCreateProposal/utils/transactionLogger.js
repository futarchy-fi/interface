...
class TransactionLogger {
  ...
  exportToJSON(filename = null, additionalData = {}) {
    ...
    // Add skipped pools from allResults if provided
    if (additionalData.allResults) {
      additionalData.allResults.forEach(result => {
        if (result.skipped && result.poolAddress) {
          // Check if not already in pools array
          if (!pools.some(p => p.poolNumber === result.poolNumber)) {
            pools.push({
              poolNumber: result.poolNumber,
              address: result.poolAddress,
              token0: result.token0,
              token1: result.token1,
              description: `Pool ${result.poolNumber} - Skipped (Existing Pool)`,
              status: 'skipped',
